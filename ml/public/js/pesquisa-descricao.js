...
async function executarPesquisaRapida() {
  ...
  try {
    ...
    const resp = await fetch('/ml/api/pesquisa-descricao/enfileirar', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(payload) });
...
async function atualizarMonitoramento() {
  try {
    // Stats
    const st = await fetch('/ml/api/pesquisa-descricao/status?_=' + Date.now(), { cache: 'no-store' });
    ...
    // Jobs
    const jb = await fetch('/ml/api/pesquisa-descricao/jobs?_=' + Date.now(), { cache: 'no-store' });
...
async function verDetalhesJob(jobId) {
  ...
  try {
    const resp = await fetch('/ml/api/pesquisa-descricao/jobs?_=' + Date.now(), { cache:'no-store' });
    const data = await resp.json();
